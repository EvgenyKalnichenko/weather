<template>
  <div class="card-weather">
    <div v-if="value.name" class="card-weather__top">{{ value.name }}, {{ value.sys.country }}</div>
    ...
    <div class="card-weather__row">
      <!-- <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" width="34px" height="34px" viewBox="0 0 14 14">
        ...
        Feels like: {{ value.main.feels_like }}°C, {{ value.weather[0].description }}
    </div>
    <div class="card-weather__grid">
      <div class="card-weather__grid-item"><PressureIcon />{{ value.main.pressure }} hPa</div>
      <div class="card-weather__grid-item"><WindIcon /> {{ value.wind.speed }} m/s</div>
      <div class="card-weather__grid-item"><HumibityIcon /> {{ value.main.humidity }} %</div>
      <div class="card-weather__grid-item"><EyeIcon /> {{ value.visibility }} m</div>
    </div>
  </div>
</template>
...
import HumibityIcon from '@/components/icons/humidity.vue'
